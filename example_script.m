...
%% initialize
native_res = mean(diff(wl))             %spectral interval of input data (should be monotonous)
...
wlFpRes= [wl(1):fingerprint_res:wl(end)]'; %wavelength grid corresponding to fingerprint_res

bandwidth_opt=round(7.5/native_res);    %proper for 3.3 nm spaced bands (Ramses, Hypersas). Higher resolution -> increase value to approximately bandwidth = round(7.5/resolution)
...
rho_fingerprint_EmptyFlag = zeros(numel(ID),1); %raised if the fingerprint returns no band indices. Rho_s will be NaN.
...
rangeexclude = {1:bandwidth_opt;...
                find(wl<=750,1,'last'):find(wl>=780,1,'first');...
                numel(wl)-bandwidth_opt+1:numel(wl)};
rangeexcludemask = ones(size(wl));
rangeexcludemask(cell2mat(rangeexclude'))=0;

% spectral domain where negative Rrs must not occur, this will also set the upper limit of rho_s:
nonnegrange = [find(wl>=400,1,'first'):1:find(wl>=700,1,'first')]; % spectral range where Rrs is not allowed to become negative
...
for i = 1:numel(ID);
    ...
    rho_fingerprint_ind{i,1} = rho_fingerprint_getfingerprint(tmpLtFpRes,tmpLsFpRes,featureseparator);
